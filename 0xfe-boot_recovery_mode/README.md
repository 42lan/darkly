...
1. Run the VM
2. Press `Tab` to edit boot options
3. Edit boot parameters line from ... to below
```sh
/casper/vmlinuz file=/cdrom/preseed/custom.seed boot=casper initrd=/casper/initrd.gz quiet splash --
```
```sh
/casper/vmlinuz file=/cdrom/preseed/custom.seed boot=casper initrd=/casper/initrd.gz quiet splash init=/bin/bash
```
4. Press `Return` to boot the live system
5. Search for 64 bytes alphanumerical strings within `/var/www/html` folder
```sh
root@BornToSecWeb:/# grep -roE "[[:alnum:]]{64}" /var/www/ 2>/dev/null
